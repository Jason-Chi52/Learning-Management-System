...
export default function InstructorCoursesPage() {
  ...
  async function fetchCourses() {
    setLoading(true);
    try {
      const res = await fetch(`${API}/courses/`); // GET doesn't need auth
  ...
  async function createCourse(e: React.FormEvent) {
    ...
    const res = await fetch(`${API}/courses/`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeaders() }, // keep simple
            ...
            {editingId === c.id ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => { setEditingId(c.id); setForm({ title: c.title, description: c.description ?? '' }); }}>
                    Edit
                  </button>
